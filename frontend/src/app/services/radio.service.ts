...
export class RadioService {
  ...
  getRadio(email: string, idSejour: number): Observable<any> {
     if (typeof window !== 'undefined' && window.localStorage) {
          ...
          const url = `http://127.0.0.1:8000/profile/${encodeURIComponent(email)}/${encodeURIComponent(idSejour)}/labs/Radiology`;

          console.log('Fetching Radio from URL:', url);
